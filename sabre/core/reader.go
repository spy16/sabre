...
// Token reads one token from the reader and returns. If init is positive,
// it is included as first character in the token.
...
// Container reads multiple forms until 'end' rune is reached. Should be used
// to read collection types like List etc. formType is only used to annotate
// errors.
func (rd Reader) Container(end rune, formType string) ([]Value, error) {
	var forms []Value
...
func (rd *Reader) annotateErr(e error) error {
	...
	return NewErr(true, rd.Position(), e)
}
...
func readUnicodeChar(token string, base int) (Char, error) {
